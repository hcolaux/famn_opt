# -*- coding: utf-8 -*-
'''
Program to run FAM-N optimisation, version 0.3a

FAM-N (Fast Amplitude Modulation - N pulses) is a composite pulse that
exploits the phenomenon of inversion/saturation of satellite transtion into the central
transition to enhance the efficiency of the conversion pulse in MQMAS
experiments, improving the overall sensitivity of the experiment.
This program determines the number of individual pulse and their
respective durations required for an optimal conversion under the
experimental conditions defined by the user.

To run the optimisation, write down all requested parameters below as described
in the comment session and press the "Run" button on the Matlab window. 

- More information: 
Colaux, H.; Dawson, D. M. & Ashbrook, S. E.,
Efficient Ampli tude-Modulated Pulses for Triple-to Single-Quantum Coherence Conversion in MQMAS NMR,
...
- More information: 
Bak, M.; Rasmussen, J.; Nielsen, N. & others,
...
This file is part of FAMN optimisation program.

FAMN optimisation program is free software: you can redistribute it and/or modify
it under the terms of the GNU General Public License as published by
the Free Software Foundation, version 3.

FAMN optimisation program is distributed in the hopeq that it will be useful,
but without any warranty; without even the implied warranty of
 merchantability or fitness for a particular purpose.  See the
GNU General Public License for more details.

You should have received a copy of the GNU General Public License
along with FAMN optimisation program.  If not, see <http://www.gnu.org/licenses/>.
...
class FAMN_main:
    ...
    def __init__(self): 
        ...
        self.param = { # ---------------------------------- 
                ...
                'cryst_gen_method' : 'golden', # Method used to genenerate the crystal file. Can be 'golden', 'zcw', or 'random'
                ...
                'no_SIMPSON_execution' : True, # Disable simpson execution
    ...
    class FAMN_step():
        ...
        def __init__(self,FAMN_main):
            ...
            self.temp = { # Temporary variables
                    ...
                    'suffix' : '', #
                    'set_filename' : '', # Function that sets the filename on every step
...
#                simpson_file.write('-x\n')
        ...
        def final_simulation(self):
            ...
            # Set the function that defines the file name
            if self.FAMN_main.param['keep_simpson_files']: # Name changes at every iteration
                self.temp['set_filename'] = self.finalise_filename
            else: # Name is kept constant
                self.temp['set_filename'] = self.fixed_filename
    ...
    def initialise(self):
        ...
        if (not self.param['SIMPSON_installed']
            and self.param['no_SIMPSON_execution']):
            if self.param['console_warning']:
                    ...
                    try: # Verifies if the number of crystallite is integer
                        ...
                        if self.param['no_cryst'] < 2: 
                            ...
                            self.param['crystfile'] = ( './' + self.param['simpson_tempfolder'] +\
                                           '/' + self.param['cryst_name'] ) # Crystal file used for the SIMPSON optimisation
    ...
    def finalise(self):
        ...
        # Export automatically if the output path is specified
        if self.param['export_filepath'] != '.':
            self.export_all()
            print('Exporting output to', self.param['export_filepath'])
...
#            report.write(str(self.output['list_detect_density']) + '\n')
    ...
    def select_folder(self):
        '''Select folder for the output of things'''
        self.param['export_filepath'] = fmc.select_directory('Select output folder')
    ...
    def export_all(self):
        ...
        if self.param['export_filepath'] != '.':
            ...
            self.export_text_report((self.param['simpson_basename'] + '_text_report'),
                                      self.param['export_filepath'])
            
            self.plot_allsteps_fig()
            self.figures['allsteps_fig'].savefig(self.param['export_filepath'] +
                        os.sep + self.param['simpson_basename'] + '.pdf')
            ...
            self.export_topspin_MQF((self.param['simpson_basename'] + '_topspin_MQF'),
                                          self.param['export_filepath'])
            self.export_topspin_MQCPMGMAS((self.param['simpson_basename'] + 'topspin_MQCPMGMAS'),
                                          self.param['export_filepath'])
    ...
    def save(self):
        ''' Save the current state of this class instance as '''
        # Get file path
        ...
        if inputpath != None:
            ...
            print('Instant', filename ,'successfully saved in', filepath)
